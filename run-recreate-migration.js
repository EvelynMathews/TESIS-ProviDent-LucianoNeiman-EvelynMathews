...
/**
 * Script temporal para RECREAR la tabla addresses con nueva estructura
...
async function runMigration() {
  try {
    ...
    const sqlFile = join(__dirname, 'recreate-addresses-migration.sql')
    const sql = readFileSync(sqlFile, 'utf-8')

    console.log('⚠️  ADVERTENCIA: Esto eliminará la tabla addresses existente y todos sus datos')
    console.log('🚀 Ejecutando migración en 2 segundos...')
    ...
    console.log('\n📋 Próximos pasos:')
    console.log('1. Verificá en Supabase Dashboard → Table Editor que la tabla "addresses" tiene la nueva estructura:')
    console.log('   - street, city, province, postal_code, country, is_primary')
    console.log('2. ⚠️  BORRAR la línea SUPABASE_SERVICE_ROLE_KEY del archivo .env.local')
    console.log('3. Borrar este archivo (run-recreate-migration.js) y el anterior (run-migration.js)')

  } catch (error) {
    console.error('\n❌ Error en migración:', error.message)
    console.log('\n💡 Solución alternativa:')
    console.log('Ejecutá el SQL manualmente en Supabase Dashboard → SQL Editor')
    console.log('Archivo: recreate-addresses-migration.sql')
    process.exit(1)
  }
}
